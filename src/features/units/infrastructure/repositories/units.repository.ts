...
import type { UnitRepository } from "../../application/ports/units.repository";
import type { CursorPage, MasterDataListQuery, UnitCreateInput, UnitRecord, UnitUpdateInput } from "../../application/types";

const TABLE_NAME = "units";
const SELECT_COLUMNS = "id, tenant_id, branch_id, code, name_ar, name_en, unit_type, precision_scale, is_base_unit, is_active, created_at, updated_at, version";
const SEARCH_COLUMNS = ["code","name_ar","name_en"];

function mapRow(row: Record<string, unknown>): UnitRecord {
  return {
    id: row.id as string,
    ...
    precisionScale: row.precision_scale as number,
    isBaseUnit: row.is_base_unit as boolean,
...
function normalizePayload(input: UnitCreateInput | UnitUpdateInput, context: TenantRequestContext) {
  const payload: Record<string, unknown> = {
    ...
    unit_type: normalizeValue(input.unitType),
    precision_scale: Number(input.precisionScale ?? 2),
    is_base_unit: input.isBaseUnit === true,
  };
...
function encodeCursor(record: UnitRecord | undefined) {
...
export class SupabaseUnitRepository implements UnitRepository {
  ...
  async list(query: MasterDataListQuery): Promise<CursorPage<UnitRecord>> {
    ...
    if (error) {
      throw new ApplicationError({ code: "OPERATIONAL_ERROR", message: "Could not list Units.", cause: error });
    }
  ...
  async findById(id: string): Promise<UnitRecord | null> {
    ...
    if (error) {
      throw new ApplicationError({ code: "OPERATIONAL_ERROR", message: "Could not read Unit.", cause: error });
    }
  ...
  async create(input: UnitCreateInput): Promise<UnitRecord> {
    ...
    if (error) {
      throw new ApplicationError({ code: "OPERATIONAL_ERROR", message: "Could not create Unit.", cause: error });
    }

    return mapRow(data);
  }

  async update(id: string, input: UnitUpdateInput): Promise<UnitRecord> {
    const { data, error } = await this.supabase
      .from(TABLE_NAME)
    ...
    if (error) {
      throw new ApplicationError({ code: "OPERATIONAL_ERROR", message: "Could not update Unit.", cause: error });
    }
  ...
  async softDelete(id: string): Promise<void> {
    ...
    if (error) {
      throw new ApplicationError({ code: "OPERATIONAL_ERROR", message: "Could not soft delete Unit.", cause: error });
    }
  }
}
