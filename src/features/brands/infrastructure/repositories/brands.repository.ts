...
import type { BrandRepository } from "../../application/ports/brands.repository";
import type { CursorPage, MasterDataListQuery, BrandCreateInput, BrandRecord, BrandUpdateInput } from "../../application/types";

const TABLE_NAME = "brands";
const SELECT_COLUMNS = "id, tenant_id, branch_id, code, name_ar, name_en, is_active, created_at, updated_at, version";
const SEARCH_COLUMNS = ["code","name_ar","name_en"];

function mapRow(row: Record<string, unknown>): BrandRecord {
  return {
    id: row.id as string,
...
function normalizePayload(input: BrandCreateInput | BrandUpdateInput, context: TenantRequestContext) {
...
function encodeCursor(record: BrandRecord | undefined) {
...
export class SupabaseBrandRepository implements BrandRepository {
  ...
  async list(query: MasterDataListQuery): Promise<CursorPage<BrandRecord>> {
    ...
    if (error) {
      throw new ApplicationError({ code: "OPERATIONAL_ERROR", message: "Could not list Brands.", cause: error });
    }
  ...
  async findById(id: string): Promise<BrandRecord | null> {
    ...
    if (error) {
      throw new ApplicationError({ code: "OPERATIONAL_ERROR", message: "Could not read Brand.", cause: error });
    }
  ...
  async create(input: BrandCreateInput): Promise<BrandRecord> {
    ...
    if (error) {
      throw new ApplicationError({ code: "OPERATIONAL_ERROR", message: "Could not create Brand.", cause: error });
    }

    return mapRow(data);
  }

  async update(id: string, input: BrandUpdateInput): Promise<BrandRecord> {
    const { data, error } = await this.supabase
      .from(TABLE_NAME)
    ...
    if (error) {
      throw new ApplicationError({ code: "OPERATIONAL_ERROR", message: "Could not update Brand.", cause: error });
    }
  ...
  async softDelete(id: string): Promise<void> {
    ...
    if (error) {
      throw new ApplicationError({ code: "OPERATIONAL_ERROR", message: "Could not soft delete Brand.", cause: error });
    }
  }
}
