...
import type { ProductCategoryRepository } from "../../application/ports/product-categories.repository";
import type { CursorPage, MasterDataListQuery, ProductCategoryCreateInput, ProductCategoryRecord, ProductCategoryUpdateInput } from "../../application/types";

const TABLE_NAME = "product_categories";
const SELECT_COLUMNS = "id, tenant_id, branch_id, code, name_ar, name_en, parent_category_id, is_active, created_at, updated_at, version";
const SEARCH_COLUMNS = ["code","name_ar","name_en"];

function mapRow(row: Record<string, unknown>): ProductCategoryRecord {
  return {
    id: row.id as string,
    ...
    parentCategoryId: row.parent_category_id as string | null,
...
function normalizePayload(input: ProductCategoryCreateInput | ProductCategoryUpdateInput, context: TenantRequestContext) {
  const payload: Record<string, unknown> = {
    ...
    parent_category_id: normalizeValue(input.parentCategoryId),
  };
...
function encodeCursor(record: ProductCategoryRecord | undefined) {
...
export class SupabaseProductCategoryRepository implements ProductCategoryRepository {
  ...
  async list(query: MasterDataListQuery): Promise<CursorPage<ProductCategoryRecord>> {
    ...
    if (error) {
      throw new ApplicationError({ code: "OPERATIONAL_ERROR", message: "Could not list Product Categories.", cause: error });
    }
  ...
  async findById(id: string): Promise<ProductCategoryRecord | null> {
    ...
    if (error) {
      throw new ApplicationError({ code: "OPERATIONAL_ERROR", message: "Could not read ProductCategory.", cause: error });
    }
  ...
  async create(input: ProductCategoryCreateInput): Promise<ProductCategoryRecord> {
    ...
    if (error) {
      throw new ApplicationError({ code: "OPERATIONAL_ERROR", message: "Could not create ProductCategory.", cause: error });
    }

    return mapRow(data);
  }

  async update(id: string, input: ProductCategoryUpdateInput): Promise<ProductCategoryRecord> {
    const { data, error } = await this.supabase
      .from(TABLE_NAME)
    ...
    if (error) {
      throw new ApplicationError({ code: "OPERATIONAL_ERROR", message: "Could not update ProductCategory.", cause: error });
    }
  ...
  async softDelete(id: string): Promise<void> {
    ...
    if (error) {
      throw new ApplicationError({ code: "OPERATIONAL_ERROR", message: "Could not soft delete ProductCategory.", cause: error });
    }
  }
}
