...
import type { SupplierRepository } from "../../application/ports/suppliers.repository";
import type { CursorPage, MasterDataListQuery, SupplierCreateInput, SupplierRecord, SupplierUpdateInput } from "../../application/types";

const TABLE_NAME = "suppliers";
const SELECT_COLUMNS = "id, tenant_id, branch_id, supplier_code, name_ar, name_en, phone, email, supplier_type, is_active, created_at, updated_at, version";
const SEARCH_COLUMNS = ["supplier_code","name_ar","name_en","phone","email","tax_number"];

function mapRow(row: Record<string, unknown>): SupplierRecord {
  return {
    id: row.id as string,
...
function normalizePayload(input: SupplierCreateInput | SupplierUpdateInput, context: TenantRequestContext) {
  const payload: Record<string, unknown> = {
    tenant_id: context.tenantId,
    updated_by: context.userId,
    supplier_code: normalizeValue(input.supplierCode),
    ...
    tax_number: normalizeValue(input.taxNumber),
    supplier_type: normalizeValue(input.supplierType),
  };
...
function encodeCursor(record: SupplierRecord | undefined) {
...
export class SupabaseSupplierRepository implements SupplierRepository {
  ...
  async list(query: MasterDataListQuery): Promise<CursorPage<SupplierRecord>> {
    ...
    if (error) {
      throw new ApplicationError({ code: "OPERATIONAL_ERROR", message: "Could not list Suppliers.", cause: error });
    }
  ...
  async findById(id: string): Promise<SupplierRecord | null> {
    ...
    if (error) {
      throw new ApplicationError({ code: "OPERATIONAL_ERROR", message: "Could not read Supplier.", cause: error });
    }

    return data ? mapRow(data) : null;
  }

  async create(input: SupplierCreateInput): Promise<SupplierRecord> {
    const { data, error } = await this.supabase
      .from(TABLE_NAME)
    ...
    if (error) {
      throw new ApplicationError({ code: "OPERATIONAL_ERROR", message: "Could not create Supplier.", cause: error });
    }

    return mapRow(data);
  }

  async update(id: string, input: SupplierUpdateInput): Promise<SupplierRecord> {
    const { data, error } = await this.supabase
      .from(TABLE_NAME)
    ...
    if (error) {
      throw new ApplicationError({ code: "OPERATIONAL_ERROR", message: "Could not update Supplier.", cause: error });
    }
  ...
  async softDelete(id: string): Promise<void> {
    ...
    if (error) {
      throw new ApplicationError({ code: "OPERATIONAL_ERROR", message: "Could not soft delete Supplier.", cause: error });
    }
  }
}
