import { defineAppManifest, type AppManifest } from "@/platform/app-registry/public-api";

import { INVENTORY_PERMISSIONS } from "./permissions/permission-registry";

export const inventoryAppManifest = defineAppManifest({
  capabilities: [
    { key: "inventory.navigation", requiredPermission: INVENTORY_PERMISSIONS.productsView, type: "navigation" },
    { key: "inventory.search", requiredPermission: INVENTORY_PERMISSIONS.searchView, type: "search" },
    { key: "inventory.reports", requiredPermission: INVENTORY_PERMISSIONS.reportsView, type: "report" },
    { key: "inventory.prints", requiredPermission: INVENTORY_PERMISSIONS.reportsView, type: "print" },
    { key: "inventory.dashboards", requiredPermission: INVENTORY_PERMISSIONS.reportsView, type: "dashboard" },
    { key: "inventory.import-export", requiredPermission: INVENTORY_PERMISSIONS.importExportManage, type: "connector" },
    { key: "inventory.document-contracts", requiredPermission: INVENTORY_PERMISSIONS.movementsView, type: "workflow" },
    { key: "inventory.notifications", requiredPermission: INVENTORY_PERMISSIONS.reservationsView, type: "notification" },
  ],
  category: "operations",
  commands: [
    {
      actionType: "open-page",
      appKey: "inventory",
      category: "navigation",
      href: "/erp/inventory/products",
      key: "inventory.products.open",
      label: "Open Product Master",
      order: 30,
      requiredPermission: INVENTORY_PERMISSIONS.productsView,
      scope: "global",
      supportedExperiences: ["erp"],
    },
    {
      actionType: "open-page",
      appKey: "inventory",
      category: "navigation",
      href: "/erp/inventory",
      key: "inventory.open",
      label: "Open Inventory Foundation",
      order: 31,
      requiredPermission: INVENTORY_PERMISSIONS.productsView,
      scope: "global",
      supportedExperiences: ["erp"],
    },
    {
      actionType: "open-page",
      appKey: "inventory",
      category: "navigation",
      href: "/erp/inventory/documentation",
      key: "inventory.documentation.open",
      label: "Open Inventory Documentation",
      order: 32,
      requiredPermission: INVENTORY_PERMISSIONS.productsView,
      scope: "global",
      supportedExperiences: ["erp"],
    },
  ],
  dashboards: [
    { key: "inventory.foundation.dashboard-template", requiredPermission: INVENTORY_PERMISSIONS.reportsView, type: "dashboard" },
  ],
  dependencies: [
    { appKey: "platform", reason: "Relies on Platform v1.0 app registry, security, data/RLS, document, event, search, import/export, report, print, dashboard, audit, job, and cost contracts." },
    { appKey: "finance", reason: "Consumes accepted Finance Foundation posting-readiness contracts without accounting postings." },
  ],
  description: "Foundation-only Inventory app for products, UOMs, warehouses, stock document contracts, quantities, reservations, reorder definitions, and integration readiness.",
  experiences: ["erp"],
  featureFlags: [],
  icon: "boxes",
  key: "inventory",
  name: "Inventory Foundation",
  navigation: [
    {
      appKey: "inventory",
      href: "/erp/inventory",
      key: "inventory.launcher",
      kind: "app",
      label: "Inventory",
      order: 30,
      placement: "app-launcher",
      requiredPermission: INVENTORY_PERMISSIONS.productsView,
      supportedExperiences: ["erp"],
    },
    {
      appKey: "inventory",
      href: "/erp/inventory",
      key: "inventory.sidebar",
      label: "Inventory Foundation",
      order: 30,
      placement: "contextual-sidebar",
      requiredPermission: INVENTORY_PERMISSIONS.productsView,
      supportedExperiences: ["erp"],
    },
  ],
  permissions: [
    INVENTORY_PERMISSIONS.productsView,
    INVENTORY_PERMISSIONS.uomsView,
    INVENTORY_PERMISSIONS.warehousesView,
    INVENTORY_PERMISSIONS.handlingUnitsView,
    INVENTORY_PERMISSIONS.stockView,
  ],
  prints: [
    { key: "inventory.foundation.readiness-print", requiredPermission: INVENTORY_PERMISSIONS.reportsView, type: "print" },
  ],
  quickActions: [],
  reports: [
    { key: "inventory.foundation.readiness", requiredPermission: INVENTORY_PERMISSIONS.reportsView, type: "report" },
  ],
  notifications: [
    { key: "inventory.foundation.notification-readiness", requiredPermission: INVENTORY_PERMISSIONS.reservationsView, type: "notification" },
  ],
  routes: [
    {
      experience: "erp",
      key: "inventory.home",
      label: "Inventory Foundation",
      path: "/erp/inventory",
      requiredPermission: INVENTORY_PERMISSIONS.productsView,
    },
    {
      experience: "erp",
      key: "inventory.products",
      label: "Product Master",
      path: "/erp/inventory/products",
      requiredPermission: INVENTORY_PERMISSIONS.productsView,
    },
    {
      experience: "erp",
      key: "inventory.documentation",
      label: "Inventory Documentation",
      path: "/erp/inventory/documentation",
      requiredPermission: INVENTORY_PERMISSIONS.productsView,
    },
    {
      experience: "erp",
      key: "inventory.categories",
      label: "Inventory Categories",
      path: "/erp/inventory/categories",
      requiredPermission: INVENTORY_PERMISSIONS.productsView,
    },
    {
      experience: "erp",
      key: "inventory.variants",
      label: "Inventory Variants",
      path: "/erp/inventory/variants",
      requiredPermission: INVENTORY_PERMISSIONS.productsView,
    },
    {
      experience: "erp",
      key: "inventory.uom-categories",
      label: "Inventory UOM Categories",
      path: "/erp/inventory/uom-categories",
      requiredPermission: INVENTORY_PERMISSIONS.uomsView,
    },
    {
      experience: "erp",
      key: "inventory.uoms",
      label: "Inventory UOMs",
      path: "/erp/inventory/uoms",
      requiredPermission: INVENTORY_PERMISSIONS.uomsView,
    },
    {
      experience: "erp",
      key: "inventory.warehouses",
      label: "Inventory Warehouses",
      path: "/erp/inventory/warehouses",
      requiredPermission: INVENTORY_PERMISSIONS.warehousesView,
    },
    {
      experience: "erp",
      key: "inventory.locations",
      label: "Inventory Locations",
      path: "/erp/inventory/locations",
      requiredPermission: INVENTORY_PERMISSIONS.locationsView,
    },
    {
      experience: "erp",
      key: "inventory.lots",
      label: "Inventory Lots",
      path: "/erp/inventory/lots",
      requiredPermission: INVENTORY_PERMISSIONS.lotsView,
    },
    {
      experience: "erp",
      key: "inventory.serials",
      label: "Inventory Serials",
      path: "/erp/inventory/serials",
      requiredPermission: INVENTORY_PERMISSIONS.serialsView,
    },
    {
      experience: "erp",
      key: "inventory.reorder-rules",
      label: "Inventory Reorder Rules",
      path: "/erp/inventory/reorder-rules",
      requiredPermission: INVENTORY_PERMISSIONS.reorderRulesView,
    },
    {
      experience: "erp",
      key: "inventory.handling-unit-types",
      label: "Handling Unit Types",
      path: "/erp/inventory/handling-unit-types",
      requiredPermission: INVENTORY_PERMISSIONS.handlingUnitsView,
    },
    {
      experience: "erp",
      key: "inventory.handling-units",
      label: "Handling Units",
      path: "/erp/inventory/handling-units",
      requiredPermission: INVENTORY_PERMISSIONS.handlingUnitsView,
    },
    {
      experience: "erp",
      key: "inventory.handling-unit-contents",
      label: "Handling Unit Contents",
      path: "/erp/inventory/handling-unit-contents",
      requiredPermission: INVENTORY_PERMISSIONS.handlingUnitsView,
    },
    {
      experience: "erp",
      key: "inventory.ledger",
      label: "Inventory Ledger",
      path: "/erp/inventory/ledger",
      requiredPermission: INVENTORY_PERMISSIONS.stockView,
    },
    {
      experience: "erp",
      key: "inventory.reservations",
      label: "Inventory Reservations",
      path: "/erp/inventory/reservations",
      requiredPermission: INVENTORY_PERMISSIONS.reservationsView,
    },
  ],
  sensitiveData: "sensitive",
  settings: [
    { key: "inventory.foundation.settings", requiredPermission: INVENTORY_PERMISSIONS.warehousesManage, type: "setting" },
  ],
  statuses: ["draft", "active", "inactive", "locked", "archived"],
  version: "1.0.0",
} satisfies AppManifest);
