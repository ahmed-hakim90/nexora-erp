...
import type { PriceListRepository } from "../../application/ports/price-lists.repository";
import type { CursorPage, MasterDataListQuery, PriceListCreateInput, PriceListRecord, PriceListUpdateInput } from "../../application/types";

const TABLE_NAME = "price_lists";
const SELECT_COLUMNS = "id, tenant_id, branch_id, code, name_ar, name_en, price_list_type, currency_code, is_active, created_at, updated_at, version";
const SEARCH_COLUMNS = ["code","name_ar","name_en","currency_code"];

function mapRow(row: Record<string, unknown>): PriceListRecord {
  return {
    id: row.id as string,
    ...
    priceListType: row.price_list_type as string,
...
function normalizePayload(input: PriceListCreateInput | PriceListUpdateInput, context: TenantRequestContext) {
  const payload: Record<string, unknown> = {
    ...
    price_list_type: normalizeValue(input.priceListType),
    currency_code: normalizeValue(input.currencyCode),
  };
...
function encodeCursor(record: PriceListRecord | undefined) {
...
export class SupabasePriceListRepository implements PriceListRepository {
  ...
  async list(query: MasterDataListQuery): Promise<CursorPage<PriceListRecord>> {
    ...
    if (error) {
      throw new ApplicationError({ code: "OPERATIONAL_ERROR", message: "Could not list Price Lists.", cause: error });
    }
  ...
  async findById(id: string): Promise<PriceListRecord | null> {
    ...
    if (error) {
      throw new ApplicationError({ code: "OPERATIONAL_ERROR", message: "Could not read PriceList.", cause: error });
    }

    return data ? mapRow(data) : null;
  }

  async create(input: PriceListCreateInput): Promise<PriceListRecord> {
    const { data, error } = await this.supabase
      .from(TABLE_NAME)
    ...
    if (error) {
      throw new ApplicationError({ code: "OPERATIONAL_ERROR", message: "Could not create PriceList.", cause: error });
    }

    return mapRow(data);
  }

  async update(id: string, input: PriceListUpdateInput): Promise<PriceListRecord> {
    const { data, error } = await this.supabase
      .from(TABLE_NAME)
    ...
    if (error) {
      throw new ApplicationError({ code: "OPERATIONAL_ERROR", message: "Could not update PriceList.", cause: error });
    }
  ...
  async softDelete(id: string): Promise<void> {
    ...
    if (error) {
      throw new ApplicationError({ code: "OPERATIONAL_ERROR", message: "Could not soft delete PriceList.", cause: error });
    }
  }
}
