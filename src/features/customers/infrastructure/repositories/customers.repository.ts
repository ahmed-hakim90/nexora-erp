...
import type { CustomerRepository } from "../../application/ports/customers.repository";
import type { CursorPage, MasterDataListQuery, CustomerCreateInput, CustomerRecord, CustomerUpdateInput } from "../../application/types";

const TABLE_NAME = "customers";
const SELECT_COLUMNS = "id, tenant_id, branch_id, customer_code, name_ar, name_en, phone, email, customer_type, is_active, created_at, updated_at, version";
const SEARCH_COLUMNS = ["customer_code","name_ar","name_en","phone","email","tax_number"];
...
function normalizePayload(input: CustomerCreateInput | CustomerUpdateInput, context: TenantRequestContext) {
  const payload: Record<string, unknown> = {
    tenant_id: context.tenantId,
    updated_by: context.userId,
    customer_code: normalizeValue(input.customerCode),
    ...
    customer_type: normalizeValue(input.customerType),
  };
...
function encodeCursor(record: CustomerRecord | undefined) {
...
export class SupabaseCustomerRepository implements CustomerRepository {
  ...
  async list(query: MasterDataListQuery): Promise<CursorPage<CustomerRecord>> {
    ...
    if (error) {
      throw new ApplicationError({ code: "OPERATIONAL_ERROR", message: "Could not list Customers.", cause: error });
    }
  ...
  async findById(id: string): Promise<CustomerRecord | null> {
    ...
    if (error) {
      throw new ApplicationError({ code: "OPERATIONAL_ERROR", message: "Could not read Customer.", cause: error });
    }
  ...
  async create(input: CustomerCreateInput): Promise<CustomerRecord> {
    ...
    if (error) {
      throw new ApplicationError({ code: "OPERATIONAL_ERROR", message: "Could not create Customer.", cause: error });
    }

    return mapRow(data);
  }

  async update(id: string, input: CustomerUpdateInput): Promise<CustomerRecord> {
    const { data, error } = await this.supabase
      .from(TABLE_NAME)
    ...
    if (error) {
      throw new ApplicationError({ code: "OPERATIONAL_ERROR", message: "Could not update Customer.", cause: error });
    }
  ...
  async softDelete(id: string): Promise<void> {
    ...
    if (error) {
      throw new ApplicationError({ code: "OPERATIONAL_ERROR", message: "Could not soft delete Customer.", cause: error });
    }
  }
}
