...
import type { WarehouseRepository } from "../../application/ports/warehouses.repository";
import type { CursorPage, MasterDataListQuery, WarehouseCreateInput, WarehouseRecord, WarehouseUpdateInput } from "../../application/types";

const TABLE_NAME = "warehouses";
const SELECT_COLUMNS = "id, tenant_id, branch_id, warehouse_code, name_ar, name_en, warehouse_type, branch_id, is_active, created_at, updated_at, version";
const SEARCH_COLUMNS = ["warehouse_code","name_ar","name_en"];

function mapRow(row: Record<string, unknown>): WarehouseRecord {
  return {
    id: row.id as string,
...
function normalizePayload(input: WarehouseCreateInput | WarehouseUpdateInput, context: TenantRequestContext) {
  const payload: Record<string, unknown> = {
    tenant_id: context.tenantId,
    updated_by: context.userId,
    warehouse_code: normalizeValue(input.warehouseCode),
    name_ar: normalizeValue(input.nameAr),
    name_en: normalizeValue(input.nameEn),
    warehouse_type: normalizeValue(input.warehouseType),
    branch_id: normalizeValue(input.branchId),
  };
...
function encodeCursor(record: WarehouseRecord | undefined) {
  if (!record) return null;
  return Buffer.from(JSON.stringify({ createdAt: record.createdAt, id: record.id })).toString("base64url");
}

export class SupabaseWarehouseRepository implements WarehouseRepository {
  constructor(
    private readonly supabase: SupabaseClient,
    private readonly context: TenantRequestContext,
  ) {}

  async list(query: MasterDataListQuery): Promise<CursorPage<WarehouseRecord>> {
    ...
    if (error) {
      throw new ApplicationError({ code: "OPERATIONAL_ERROR", message: "Could not list Warehouses.", cause: error });
    }
  ...
  async findById(id: string): Promise<WarehouseRecord | null> {
    ...
    if (error) {
      throw new ApplicationError({ code: "OPERATIONAL_ERROR", message: "Could not read Warehouse.", cause: error });
    }
  ...
  async create(input: WarehouseCreateInput): Promise<WarehouseRecord> {
    ...
    if (error) {
      throw new ApplicationError({ code: "OPERATIONAL_ERROR", message: "Could not create Warehouse.", cause: error });
    }

    return mapRow(data);
  }

  async update(id: string, input: WarehouseUpdateInput): Promise<WarehouseRecord> {
    const { data, error } = await this.supabase
      .from(TABLE_NAME)
    ...
    if (error) {
      throw new ApplicationError({ code: "OPERATIONAL_ERROR", message: "Could not update Warehouse.", cause: error });
    }
  ...
  async softDelete(id: string): Promise<void> {
    ...
    if (error) {
      throw new ApplicationError({ code: "OPERATIONAL_ERROR", message: "Could not soft delete Warehouse.", cause: error });
    }
  }
}
