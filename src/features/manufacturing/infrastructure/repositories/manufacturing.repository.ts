...
export class SupabaseManufacturingRepository implements ManufacturingRepository {
  ...
  /**
   * MFG-01: when a manufacturing product references inventory, verify same-tenant/company scope.
   * Does not post stock — catalog identity only.
   */
  private async assertInventoryProductLink(definition: ManufacturingResourceDefinition, input: ManufacturingMutationInput) {
    ...
    const { data, error } = await this.supabase
      ...
      .select("id, is_manufacturable, name, sku")
      .eq("tenant_id", this.context.tenantId)
