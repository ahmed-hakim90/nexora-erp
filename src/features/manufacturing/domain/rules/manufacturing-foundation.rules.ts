...
/**
 * MFG-01: active manufacturing products must reference canonical inventory identity.
 * Stock quantities remain owned by Inventory — this only enforces the catalog link.
 */
export function assertManufacturingProductRules(input: ManufacturingMutationInput) {
  ...
  const requiresInventoryLink = !deferredLinkStatuses.has(status);

  if (requiresInventoryLink && !inventoryProductId) {
    throw new ApplicationError({
      code: "VALIDATION_ERROR",
