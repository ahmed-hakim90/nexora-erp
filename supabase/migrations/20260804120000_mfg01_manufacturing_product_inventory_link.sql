...
begin
  if not exists (
    select 1 from pg_constraint where conname = 'manufacturing_products_inventory_product_id_fkey'
  ) then
    alter table public.manufacturing_products
      add constraint manufacturing_products_inventory_product_id_fkey
      foreign key (inventory_product_id) references public.inventory_products(id) on delete restrict;
  end if;
...
create unique index if not exists manufacturing_products_inventory_product_uq
  ...
  where inventory_product_id is not null and deleted_at is null and coalesce(is_active, true);
