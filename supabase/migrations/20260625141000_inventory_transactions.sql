...
create policy inventory_cycle_count_lines_update_member_permission on public.inventory_cycle_count_lines for update to authenticated using (is_active = true and deleted_at is null and public.is_tenant_member(tenant_id) and exists (select 1 from public.tenants t where t.id = tenant_id and t.is_active = true and t.deleted_at is null) and public.has_permission('inventory.cycle-count.manage', tenant_id)) with check (is_active = true and deleted_at is null and public.is_tenant_member(tenant_id) and exists (select 1 from public.tenants t where t.id = tenant_id and t.is_active = true and t.deleted_at is null) and public.has_permission('inventory.cycle-count.manage', tenant_id));

create policy event_outbox_inventory_transaction_select on public.event_outbox for select to authenticated using (is_active = true and deleted_at is null and event_name in ('inventory.transaction.created', 'inventory.transaction.submitted', 'inventory.transaction.posted', 'inventory.transaction.cancelled', 'inventory.transaction.reversed', 'inventory.cycle-count.posted') and public.is_tenant_member(tenant_id) and public.has_permission('inventory.transaction.view', tenant_id));
create policy event_outbox_inventory_transaction_insert on public.event_outbox for insert to authenticated with check (is_active = true and deleted_at is null and event_name in ('inventory.transaction.created', 'inventory.transaction.submitted', 'inventory.transaction.posted', 'inventory.transaction.cancelled', 'inventory.transaction.reversed', 'inventory.cycle-count.posted') and public.is_tenant_member(tenant_id) and (public.has_permission('inventory.transaction.create', tenant_id) or public.has_permission('inventory.transaction.post', tenant_id) or public.has_permission('inventory.cycle-count.post', tenant_id)));
...
values
  ...
  ('inventory.cycle-count.view', 'View Cycle Counts', 'View Sprint 10 cycle count records.', 'standard'),
