...
test("MFG-01 active manufacturing products require inventoryProductId", () => {
  assert.throws(
    ...
    (error: unknown) => error instanceof ApplicationError && /inventory product/i.test(error.message),
  );
...
test("MFG-01 manufacturable inventory products require manufacturing role", () => {
  assert.equal(inventoryProductMutationSchema.safeParse({ ...baseInventoryProduct, isManufacturable: true }).success, false);
...
test("MFG-01 manufacturing item roles and warehouse floor map are stable", () => {
  for (const role of MANUFACTURING_ITEM_ROLE_VALUES) assert.equal(isManufacturingItemRole(role), true);
