...
> **Note:** Canonical content lives here. Previous location: `docs/ENTERPRISE_DESIGN_SYSTEM.md`.  
> This document implements the [Nexora Platform UX Constitution](NEXORA_PLATFORM_UX_CONSTITUTION.md) for visual design tokens and component styling.
...
* `EditableField` and `EditableSectionCard` for profile/detail inline editing

Reference fields must use EntityLookup.

Never use raw text inputs for foreign keys.

Never use native date inputs. See [Inline Editing Standard](INLINE_EDITING_STANDARD.md).
