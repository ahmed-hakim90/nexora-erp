# ADR-012: Current App Foundation Decisions
...
Finance, Inventory, and Manufacturing apps required locked table ownership and scope boundaries before further app work.
...
- New app work must use canonical `finance_*`, `inventory_*`, and `manufacturing_*` tables.
- Legacy compatibility tables remain read-only or migration surfaces only.
- Future apps must reuse stabilized lookup, validation, permission, and modal conventions.
...
- [Manufacturing Legacy Reconciliation](../02-business-apps/MANUFACTURING_LEGACY_RECONCILIATION.md)
